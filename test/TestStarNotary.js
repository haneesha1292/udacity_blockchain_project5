...
it('can add the star name and star symbol properly', async() => {
    ...
    let starSymbol = await instance.starSymbol.call();
    assert.equal(starName, "My Star Notary Token");
    assert.equal(starSymbol, "MSNT");
});
...
it('lets a user transfer a star', async() => {
    ...
    let user2 = accounts[2];
    let tokenId1 = 9;
    await instance.createStar("Another Haneesha's Star", tokenId1, { from: user1 });
    assert.equal(await instance.ownerOf.call(tokenId1), user1);
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let instance = await StarNotary.deployed();
    let tokenId = 6;
    await instance.createStar("Haneesha's Star!", tokenId, { from: accounts[0] });
    assert.equal(await instance.lookUptokenIdToStarInfo(tokenId), "Haneesha's Star!");
});